...
%N.nf 那个n与N都是数字，f代表floating(浮点)，例如，总共10位数，小数点有两位，即为%10.2f。
...
%8.2f表示，总共有8位字符，整数部分占5位，小数点占1位，小数部分占2位。
```

### awk:好用的数据处理工具

相比起sed常常作用于一整行的处理，awk则比较倾向于一行中分成数个「字段」来处理。
...
awk可以处理后续接的档案，也可以读取来自前个指令的standard output。awk主要是处理每一行的字段内的数据，而默认的字段的分隔符为空格键或tab键。

*awk正是我想要的!*

如果我想要取出账号与登入者的IP，且账号之间以[tab]隔开，则会变成这样：

```
last -n 5 | awk '{print $1 "\t" $3}'

$1 表示一行的第一列，$3表示第三列，以此类推
```
